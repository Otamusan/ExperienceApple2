...
public abstract class AwekenedSpawnerBaseLogic extends MobSpawnerBaseLogic {
	/** The delay to spawn. */
			...
			if (this.getSpawnerWorld().isRemote) {
				...
				for (int i = 0; i < this.spawnCount; ++i) {
					...
					if (entityliving == null || net.minecraftforge.event.ForgeEventFactory.canEntitySpawnSpawner(
							entityliving, getSpawnerWorld(), (float) entity.posX, (float) entity.posY,
							(float) entity.posZ, this)) {
						if (this.spawnData.getNbt().getSize() == 1 && this.spawnData.getNbt().hasKey("id", 8)
	...
	public Entity getCachedEntity() {
		if (this.cachedEntity == null) {
			this.cachedEntity = AnvilChunkLoader.readWorldEntity(this.spawnData.getNbt(), this.getSpawnerWorld(),
					false);
